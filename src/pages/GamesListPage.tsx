...
const GamesListPage = () => {
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Featured Game - Using the first game from centralized data */}
            ...
            <div className="bg-gradient-to-r from-red-600 to-red-500 rounded-2xl overflow-hidden shadow-2xl">
              <div className="grid lg:grid-cols-2 items-center">
                <div className="p-12 text-white">
                  <h3 className="text-4xl font-bold mb-4">{games[0].name}</h3>
                  <p className="text-xl text-red-100 mb-6">{games[0].tagline}</p>
                  ...
                  <div className="grid grid-cols-2 gap-6 mb-8">
                    <div>
                      ...
                      <div className="text-3xl font-bold">{games[0].minJackpot}</div>
                    </div>
                    <div>
                      <div className="text-red-200 text-sm">Ticket Price</div>
                      <div className="text-3xl font-bold">₦{games[0].ticketPrice}</div>
                    </div>
                  </div>
                  <Link
                    to={`/games/${games[0].id}/play`}
                  ...
                  <div className="bg-white/10 backdrop-blur-sm rounded-xl p-8 border border-white/20">
                    ...
                    <div className="space-y-4 text-white/90">
                      <div className="flex items-center gap-3">
                        ...
                        <span>Pick {games[0].maxNumbers} numbers from {games[0].numberRange}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <Calendar className="w-5 h-5" />
                        <span>{games[0].drawDays.join(' & ')} draws</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <Clock className="w-5 h-5" />
                        <span>{games[0].drawTime} draw time</span>
                      </div>
                      {games[0].bonusNumbers && (
                        <div className="flex items-center gap-3">
                          <Star className="w-5 h-5" />
                          <span>{games[0].bonusNumbers.name} for extra prizes</span>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div key={game.id} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <Link
                      to={`/games/${game.id}`}
                      className="w-full flex items-center justify-center gap-2 border-2 border-gray-300 text-gray-700 py-3 px-4 rounded-lg font-semibold hover:bg-gray-50 transition"
                    >
                      View Details
